...
class Dumpcerts(interfaces.plugins.PluginInterface):
    ...
    def _save_file(self, data: bytes, value: str, rule: str):
        ext = ".key" if rule == "pkcs" else ".crt"
        with self.open(f"{value}{ext}") as f:
            f.write(data)
    ...
    def _generator(self, physical: bool):
        ...
        if physical:
            ...
            for offset, rule_name, cert_or_pem in self.get_physical_certificates(
                ...
                value, output_bytes = self._get_value_and_bytes(rule_name, cert_or_pem)

                if output:
                    self._save_file(output_bytes, proc_name, hex(offset), rule_name)
            ...
            for offset, proc, rule_name, rsa_object in self.get_process_certificates(
                ...
                value, output_bytes = self._get_value_and_bytes(rule_name, rsa_object)

                if output:
                    self._save_file(output_bytes, value.split("_")[0], rule_name)
